...
if not os.path.exists('setting.py'):
    with open('setting.py','w') as fl:
        fl.write("FILE_SAVE_PATH = ''\n")
        fl.write("NEED_CONBINE_DICTORY_PATH = ''\n")
    print('''
...
会进入合并的列表中。配置完成后，请重新运行本程序。
'''
)
    os.system('pause')
    sys.exit()

from setting import FILE_SAVE_PATH,NEED_CONBINE_DICTORY_PATH
...
class CombineExcelFiles:
    ...
    def combine(self,flag = 0):

        # 不同的 flag 代表着不同的表格合并策略
        # 默认 0 代表着全合并
                    ...
                    if flag == 0:
                        for row in range(1,max_row+1):
                            for column in range(1,max_column+1):
                                self.ws.cell(row = self.max_row + 1,column = column).value = ws.cell(row = row,column = column).value
                            self.max_row += 1
                    elif flag == 1:
                        if self.max_row == 0:
                            for row in range(1,max_row+1):
